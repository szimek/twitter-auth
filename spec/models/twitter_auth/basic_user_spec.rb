...
describe TwitterAuth::User::Auth::HTTPBasic do
  before do
    stub_basic!
  end
  ...
  describe '#password=' do
    before do
      @user = Factory.build(:twitter_basic_user, :id => '123')
    end
  ...
  describe '.verify_credentials' do
    ...
    it 'should return a JSON hash of the user when successful' do
      hash = TwitterUser.verify_credentials('twitterman','test')
    ...
    it 'should return false when a 401 unauthorized happens' do
      ...
      TwitterUser.verify_credentials('twitterman','wrong').should be_false
    end
  end
  ...
  describe '.find_or_create_by_twitter_hash_and_password' do
    ...
    it 'should return the existing user if there is one' do
      TwitterUser.identify_or_create_from_twitter_hash_and_password({'id' => '123', 'screen_name' => 'twitterman'},'test').should == @user
    end

    it 'should update the attributes from the hash' do
      TwitterUser.identify_or_create_from_twitter_hash_and_password({'id' => 123, 'screen_name' => 'twitterman', 'name' => 'New Name'}, 'test').name.should == 'New Name'
    end

    it 'should update the password from the argument' do
      TwitterUser.identify_or_create_from_twitter_hash_and_password({'id' => '123', 'screen_name' => 'twitterman', 'name' => 'New Name'}, 'test2').password.should == 'test2'
    end

    it 'should create a user if one does not exist' do
      lambda{TwitterUser.identify_or_create_from_twitter_hash_and_password({'id' => 124, 'screen_name' => 'dude', 'name' => "Lebowski"}, 'test')}.should change(TwitterUser, :count).by(1)
    end

    it 'should assign the attributes from the hash to a created user' do
      user = TwitterUser.identify_or_create_from_twitter_hash_and_password({'id' => 124, 'screen_name' => 'dude', 'name' => "Lebowski"}, 'test')
      user.id.should == 124
  ...
  describe 'changing usernames' do
    ...
    it 'should not create a new record when a screen_name has changed' do
      lambda{TwitterUser.identify_or_create_from_twitter_hash_and_password({'id' => '123', 'screen_name' => 'dude'},'awesome')}.should_not change(TwitterUser,:count)
    end

    it 'should update the record with the new screen name' do
      TwitterUser.identify_or_create_from_twitter_hash_and_password({'id' => '123', 'screen_name' => 'dude'},'awesome').should == @user.reload
      @user.login.should == 'dude'
    end
  end
end
